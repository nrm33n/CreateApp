	...
	this.selectTool = function(toolName) {
		...
		for (var i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == toolName) {
				...
				select("#" + toolName + "sideBarItem").style("border", "2px solid yellow");
				...
				//if the tool has a preload function, run it. 
				if (toolbox.selectedTool.hasOwnProperty("preload")) {
					toolbox.selectedTool.preload();
				};
				
				//if the tool has a setup function, run it.
